...
<body>
			...
			<div id="content-container">
				...
				<div id="page-content">
					...
					<div class="panel">
						...
						<form id="teamForm" name="teamForm" action="/admin/groups/edit" class="panel-body form-horizontal form-padding"
							method="post">
							<input type="hidden" name="teamId" value="${view.teamId}">
							...
							<div class="form-group">
								...
								<div class="col-lg-7">
									...
									placeholder="최대 인원 수를 입력해 주세요 " value="${view.teamMax}">
										...
										<c:forEach items="${ interest }" var="item">
											<c:choose>
												<c:when test="${fn:substring(item.intKey, 2, 4) ne '00' }">
													<option value="${ item.intId }" <c:if test="${view.intId == item.intId}">selected</c:if>>${ item.intName }</option>
												</c:when>
											</c:choose>
										</c:forEach>
									</select>
							...
							<div class="form-group">
								...
								<div class="col-lg-7">
									<textarea class="form-control" id="teamShortContent" name="teamShortContent" rows="5" placeholder="간단한 소개글을 적어보세요.">
							...
							<div class="form-group">
								...
								<div class="col-lg-7">
									<textarea class="form-control" id="teamContent" name="teamContent" placeholder="소개글을 적어보세요.">
									${view.teamContent }
									</textarea>
	...
	<script>
		$(document).ready(function () {
			...
			// 짧은 자기소개 앞뒤 공백제거
			$('#teamShortContent').val($('#teamShortContent').val().trim());
		...
		function submitCheck() { //전송 전 입력체크11
			...
			// 소개글
			const teamContent = $('#teamContent').val($('#teamContent').val().trim());
