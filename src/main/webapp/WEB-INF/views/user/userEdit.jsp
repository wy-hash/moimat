	...
	<script>
		...
		function newPasswordCheck() {
			if (nul_chk($('#newPassword'), 8, 15)) { // 빈문자,길이체크:  nul_chk("검사할 객체, 최소길이, 최대길이") 리턴 true and false
				$('#passMsg').text("신규 패스워드는 8자이상 15자 미만입니다").css('color',
						'red');
				return true;
			} else {
		...
		function getCodeList() {
			...
			for (let i = 0; i < areaList.length; i++) {
				// 2차카테고리 문자열 생성
				areaStr += "<option value='"+ areaList[i].areaId +"' >"
						+ areaList[i].areaName + " "
						+ areaList[i].areaRegionName + "</option>";
			}
			...
			for (let i = 0; i < interList.length; i++) {
				interStr += "<option value='"+interList[i].intId+"'>"
						+ interList[i].intName + "</option>"
			}
